...
async function handleConvertSpecialQuotationToOrders(
  ...
  if (!special || typeof special !== "object") {
    return jsonResponse({ error: "特殊報價資料不完整" }, 400);
  }
  ...
  for (const item of qItems) {
    const cid = parseComboIdFromQuotationItemEdge(item.customizations_json);
    if (!cid) {
      return jsonResponse({ error: "特殊報價品項缺少 combo_id，無法拆單" }, 400);
    }
    if (!byCombo.has(cid)) byCombo.set(cid, []);
    byCombo.get(cid)!.push(item);
  }
  ...
  try {
    for (const [comboId, comboItems] of byCombo.entries()) {
      ...
      deferredNotifications.push({ orderId: orderData.id, linePayload, calendarPayload });
    }
    ...
    if (statusError || !updatedQuotation) {
      ...
      throw new Error("報價單狀態更新失敗，已取消轉單");
    }
    ...
    return jsonResponse({ success: true, order_id: createdOrderIds[0], order_ids: createdOrderIds }, 200);
  } catch (err: unknown) {
...
async function handleConvertToOrder(supabase: any, body: any) {
  ...
  if (qError || !quotation) {
    return jsonResponse({ error: "報價單不存在" }, 404);
  }
  ...
  if (qItemsError) {
    ...
    return jsonResponse({ error: "載入品項失敗" }, 500);
  }
  ...
  if (orderError || !orderData) {
    ...
    return jsonResponse({ error: "建立訂單失敗" }, 500);
  }
  ...
  for (const item of qItems || []) {
    ...
    if (itemError) {
      ...
      return await rollbackAndError(itemError.message || "建立訂單品項失敗");
    }
  }
  ...
  if (statusError || !updatedQuotation) {
    ...
    return await rollbackAndError("報價單狀態更新失敗，已取消轉單");
  }
