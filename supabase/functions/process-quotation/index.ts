...
async function rollbackCreatedOrders(supabase: any, orderIds: string[], logPrefix: string) {
  for (let i = orderIds.length - 1; i >= 0; i--) {
    ...
    if (itemDeleteError) {
      console.error(`${logPrefix} rollback order_items failed:`, itemDeleteError);
    }

    const { error: orderDeleteError } = await supabase.from("orders").delete().eq("id", oid);
    if (orderDeleteError) {
      console.error(`${logPrefix} rollback orders failed:`, orderDeleteError);
    }
  }
}
...
async function handleConvertToOrder(supabase: any, body: any) {
  ...
  try {
    ...
    await rollbackCreatedOrders(supabase, [orderData.id], "[process-quotation/convert_to_order]");
    return new Response(JSON.stringify({ error: msg }), {
      status: 500,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
